...
public class BaseballElimination {
	...
	private void processEachTeam(int k){
		...
		for(int p=0;p<num-2;p++){
			for(int q=p+1;q<num-1;q++){
				G.addEdge(new FlowEdge(V-2, count+num-2, getR(p, q, k)));
	...
	private int getR(int p, int q, int k){
		if(p<k && q<k) return g[p][q];
		if(p>=k && q<k) return g[p+1][q];
		if(p<k && q>=k)	return g[p][q+1];
		else return g[p+1][q+1];
	}
	...
	public static void main(String[] args) {
		...
		BaseballElimination division = new BaseballElimination("E:\\jwork\\algorithm\\Part2\\baseball\\teams5b.txt");
